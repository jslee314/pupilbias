...
import com.jslee.pupilbias.databinding.ItemImagesBinding
...
class PhotoGridAdapter(private val propertyOnClickListener: MarsOnClickListener): // 주생성자 선언: OnClickListener 라는 객체 변수에 값을 무조건 설정해야만 객체가 생성될 수 있도록 강제
    ...
    companion object DiffCallback : DiffUtil.ItemCallback<IrisImage>() {
        ...
        override fun areContentsTheSame(oldItem: IrisImage, newItem: IrisImage): Boolean {
     ...
     */
    class MarsOnClickListener(val clkListener: (irisImage: IrisImage) -> Unit) {
        fun onClick(irisImage:IrisImage) = clkListener(irisImage)
    }
}